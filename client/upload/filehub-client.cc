// 支持用户名登录上传指定的文件
// 代码目的是测试上传性能，这里不做md5秒传的请求
// 使用同一个文件进行上传测试，测试结果返回code:1 不影响上传性能，
...
#define   TC_HTTP_SERVER_IP "192.168.253.100"
// #define   TC_HTTP_SERVER_IP "127.0.0.1"
#define   USER_NAME "GuLu"          //用户名
...
//注意，这里是需要将文件内容读取到内存，所以注意文件的大小，不要读取那种几个G的文件
class FileUpload 
{
public:
    ...
    for (int i = 0; i < concurrent; ++i) {
        threads.push_back(std::thread([i,start_time,&user, &file_upload]() {
            httplib::MultipartFormDataItems items = {
                ...
                //  {"md5", RandomString(32), "", ""},                       //【2】//这里是md5的值，先故意修改成不同的值，因为server端的file_info表只允许唯一的md5值。
